...
def test_Part_3_schema():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT p.name, p.type, p.'notnull', p.pk FROM pragma_table_info('Albums_Part3') p;")
...
def test_Part_3_data():

    cursor = conn.cursor()
    cursor.execute("SELECT * FROM Albums_Part3;")
    user_data = cursor.fetchall()

    Part_3_resource = None

    with open('resource/DB_API.pickle', 'rb') as file:
        Part_3_resource = pickle.load(file)

    assert user_data == Part_3_resource
